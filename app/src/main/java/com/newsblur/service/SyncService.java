...
/**
 * The SyncService is based on an app architecture that tries to place network calls (especially larger calls or those called regularly) on independent
 * services, making the activity / fragment a passive receiver of its updates. This, along with data fragments for handling UI updates, throttles network access
 * and ensures the UI is passively updated and decoupled from network calls. Examples of other apps using this architecture include the NBCSportsTalk and Google
 * I/O 2011 apps.
 */
public class SyncService extends IntentService
{
  ...
  protected void onHandleIntent(Intent intent)
  {
    ...
    try
    {
      ...
      // TODO: is it ever valid for receiver to be null? if not, we could
      // factor out all
      // the checks below
      ...
      switch (intent.getIntExtra(SYNCSERVICE_TASK, -1))
      {
        ...
        case EXTRA_TASK_FEED_UPDATE:
          ...
          {
            StoriesResponse storiesForFeed = apiManager.getStoriesForFeed(intent.getStringExtra(EXTRA_TASK_FEED_ID),
                intent.getStringExtra(EXTRA_TASK_PAGE_NUMBER), (StoryOrder) intent.getSerializableExtra(EXTRA_TASK_ORDER),
                (ReadFilter) intent.getSerializableExtra(EXTRA_TASK_READ_FILTER));
        ...
        case EXTRA_TASK_MULTIFEED_UPDATE:
          ...
          {
            StoriesResponse storiesForFeeds = apiManager.getStoriesForFeeds(intent.getStringArrayExtra(EXTRA_TASK_MULTIFEED_IDS),
                intent.getStringExtra(EXTRA_TASK_PAGE_NUMBER), (StoryOrder) intent.getSerializableExtra(EXTRA_TASK_ORDER),
                (ReadFilter) intent.getSerializableExtra(EXTRA_TASK_READ_FILTER));
        ...
        case EXTRA_TASK_MULTISOCIALFEED_UPDATE:
          if (intent.getStringArrayExtra(EXTRA_TASK_MULTIFEED_IDS) != null)
          {
            SocialFeedResponse sharedStoriesForFeeds = apiManager.getSharedStoriesForFeeds(intent.getStringArrayExtra(EXTRA_TASK_MULTIFEED_IDS),
                intent.getStringExtra(EXTRA_TASK_PAGE_NUMBER));
        ...
        case EXTRA_TASK_DELETE_FEED:
          ...
          {
            // Long feedToBeDeleted =
            // intent.getLongExtra(EXTRA_TASK_FEED_ID, -1);
            // if (apiManager.deleteFeed(feedToBeDeleted,
            // intent.getStringExtra(EXTRA_TASK_FOLDER_NAME))) {
            // Uri feedUri =
            // FeedProvider.FEEDS_URI.buildUpon().appendPath(Long.toString(feedToBeDeleted)).build();
            ...
            // Toast.makeText(this,
            // getResources().getString(R.string.error_deleting_feed),
        ...
        case EXTRA_TASK_SOCIALFEED_UPDATE:
          if (!TextUtils.isEmpty(intent.getStringExtra(EXTRA_TASK_SOCIALFEED_ID))
              && !TextUtils.isEmpty(intent.getStringExtra(EXTRA_TASK_SOCIALFEED_USERNAME)))
          {
            SocialFeedResponse storiesForSocialFeed = apiManager.getStoriesForSocialFeed(intent.getStringExtra(EXTRA_TASK_SOCIALFEED_ID),
                intent.getStringExtra(EXTRA_TASK_SOCIALFEED_USERNAME), intent.getStringExtra(EXTRA_TASK_PAGE_NUMBER));
